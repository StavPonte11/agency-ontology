...
class EvaluationResult(BaseModel):
    justification: str = Field(
        description="Detailed explanation of the score based on the rubric, finding specific evidence in the response before assigning a score."
    )
    factual_accuracy: int = Field(
        description="Score from 1 to 5 measuring factual accuracy based on the provided context.",
        ge=1,
        le=5,
    )
    hebrew_language_compliance: int = Field(
        description="Score from 1 to 5 measuring whether the concepts are properly extracted primarily in Hebrew with correct acronym expansion if applicable.",
        ge=1,
        le=5,
    )
    improvement: str = Field(
        description="One specific actionable improvement for the extraction logic."
    )
...
eval_prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            """You are an expert military intelligence evaluator assessing knowledge graph extraction quality.
...
async def evaluate_extraction(
    source_text: str, extracted_concepts: str, extraction_trace_id: str
):
    """
    ...
    result: EvaluationResult = await eval_chain.ainvoke(
        {"source_text": source_text, "extracted_concepts": extracted_concepts}
    )

    print(
        f"Evaluation Complete. Accuracy: {result.factual_accuracy}/5, Hebrew Compliance: {result.hebrew_language_compliance}/5"
    )
...
async def main():
    ...
    sample_text = (
        'רמטכ"ל צה"ל, רב-אלוף הרצי הלוי, סייר בבסיס חיל רגלים והנחה על העלאת הכוננות.'
    )
